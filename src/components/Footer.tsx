        ...
        <div className="grid gap-9 lg:grid-cols-[0.9fr_1.7fr]">
            ...
            <Link href="/" className="inline-flex min-h-11 items-center gap-3">
              ...
              <div>
                ...
                <span className="text-sm font-bold text-white/74">
                  Hamro Social Network for Nepalese everywhere.
                </span>
              </div>
            </Link>
            <p className="mt-5 max-w-md text-sm font-semibold leading-7 text-white/74">
              A social media and community platform for Nepalese everywhere,
              built around posts, groups, pages, Bazaar, Hamro TV, events,
              businesses, messaging, and local discovery.
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              {[
                [
                  "Facebook",
                  "https://www.facebook.com/unepalsocialnetwork/",
                  "fa-facebook-f",
                ],
                [
                  "Instagram",
                  "https://www.instagram.com/unepalapp/",
                  "fa-instagram",
                ],
                ...
                <a
                  key={label}
                  href={href}
                  className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/8 text-white hover:bg-white/14"
                  aria-label={label}
                >
          ...
          <div className="grid gap-7 sm:grid-cols-2 lg:grid-cols-5">
            {footerGroups.map((group) => (
              <div key={group.title}>
                <h2 className="mb-4 text-xs font-extrabold uppercase tracking-[0.16em] text-white/64">
                  {group.title}
                </h2>
                <ul className="space-y-3 text-sm font-bold text-white/78">
                  {group.links.map(([label, href]) => (
                    <li key={label}>
                      <Link
                        href={href}
                        className="inline-flex min-h-11 items-center py-1 transition-colors hover:text-white"
                      >
                        {label}
                      </Link>
                    </li>
