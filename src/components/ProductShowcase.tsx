          ...
          <div className="grid items-center gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16">
            <FadeIn>
              ...
              <h2 className="mt-5 max-w-2xl text-[2rem] font-extrabold leading-[1.09] text-[#071632] sm:text-[3.35rem]">
                Find rooms, jobs, services, and local needs through your
                community.
              </h2>
              <p className="mt-5 max-w-xl text-base font-semibold leading-8 text-[#65676B]">
                Bazaar brings practical local listings and opportunities into
                one community space.
              </p>
              <div className="mt-6 flex flex-wrap gap-2">
                {marketTags.map(([label, icon], index) => (
                  <span
                    key={label}
                    className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-extrabold ${index === 0 ? "bg-brand-primary text-white" : "bg-[#F3F4F6] text-[#071632]"}`}
                  >
            ...
            <FadeIn delay={0.1}>
              ...
              >
                <div className="absolute bottom-4 left-4 max-w-[24rem] rounded-2xl border border-white/80 bg-white/88 p-4 shadow-sm backdrop-blur">
                  ...
                  <p className="mt-1 text-base font-extrabold text-[#071632]">
                    Rooms, jobs, services, and nearby items
                  </p>
                </div>
              </ArtworkCard>
            </FadeIn>
        ...
        <div className="container relative mx-auto px-4 sm:px-6">
          <div className="grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
            <FadeIn>
              ...
              <p className="mt-5 max-w-xl text-base font-semibold leading-8 text-[#65676B]">
                Videos, reels, and local stories presented with a more energetic
                media-first feel.
              </p>
              <div className="mt-6 flex flex-wrap gap-2">
                {mediaTags.map(([label, icon], index) => (
                  <span
                    key={label}
                    className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-extrabold ${index === 0 ? "bg-brand-blue text-white" : "bg-white text-[#071632]"}`}
                  >
                    <i className={`fa-solid ${icon}`} />
                    {label}
                  </span>
            ...
            <FadeIn delay={0.1}>
              <ArtworkCard
                ...
                alt="uNepal Hamro TV media artwork"
              >
                <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between gap-4 rounded-2xl border border-white/80 bg-white/88 p-4 shadow-sm backdrop-blur sm:left-auto sm:max-w-[26rem]">
                  <div>
                    ...
                    <p className="mt-1 text-base font-extrabold text-[#071632]">
                      Community videos and local moments
                    </p>
                  </div>
