...
const businesses = [
  ['Everest Learning Hub', 'Education & Training', '4.8', 'fa-graduation-cap'],
  ['Himalayan Grocers', 'Food & Retail', '4.7', 'fa-basket-shopping'],
  ['Namaste Legal Care', 'Professional Services', '4.9', 'fa-scale-balanced'],
];
        ...
        <div className="grid min-w-0 items-center gap-10 lg:grid-cols-[1.04fr_0.96fr] lg:gap-16">
              ...
              <div className="relative rounded-[1.35rem] bg-white p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#E4E6EB] pb-4">
                  <div className="min-w-0">
                    <p className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-brand-primary">Local directory</p>
                    <h3 className="mt-1 text-xl font-extrabold text-[#071632]">Business Pages</h3>
                  </div>
                  <div className="flex max-w-full flex-wrap rounded-2xl bg-[#F3F4F6] p-1 text-xs font-extrabold text-[#65676B]">
                    <span className="rounded-xl bg-white px-3 py-2 text-brand-primary shadow-sm sm:px-4">Area</span>
                ...
                <div className="mt-4 grid gap-3">
                  {businesses.map(([name, type, rating, icon], index) => (
                    <article key={name} className="rounded-[1.25rem] border border-[#E4E6EB] bg-white p-4 shadow-[0_14px_30px_-28px_rgba(7,22,50,0.55)] transition hover:-translate-y-0.5 hover:shadow-[0_24px_42px_-32px_rgba(7,22,50,0.5)]">
                      <div className="flex items-center gap-4">
                        <div className={`flex h-13 w-13 shrink-0 items-center justify-center rounded-2xl ${index === 1 ? 'bg-[#F2F5FA] text-brand-blue' : 'bg-[#FFF2F4] text-brand-primary'}`}>
                          <i className={`fa-solid ${icon} text-lg`} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <h3 className="truncate text-[1.02rem] font-extrabold text-[#071632]">{name}</h3>
                          <p className="mt-1 text-sm font-semibold text-[#65676B]">{type}</p>
                          <p className="mt-1 text-xs font-bold text-[#65676B]">
                            <i className="fa-solid fa-star mr-1 text-brand-primary" />
                            {rating} rating
                          </p>
                        </div>
                        <button className="shrink-0 rounded-full border border-brand-primary/25 bg-white px-3 py-2 text-xs font-extrabold text-brand-primary transition hover:bg-[#FFF2F4] sm:px-4">
                          Follow
                        </button>
                      </div>
                    </article>
          ...
          <FadeIn delay={0.1} className="min-w-0">
            ...
            <p className="mt-5 max-w-2xl text-base font-semibold leading-8 text-[#65676B]">
              Create a page, list a business, share updates, receive messages, and make local services easier to find.
            </p>
            <div className="mt-7 grid grid-cols-2 gap-2 sm:gap-3">
              {actions.map(([label, icon]) => (
                <div key={label} className="flex min-h-24 flex-col justify-between rounded-[1.15rem] border border-[#E4E6EB] bg-white p-3 font-extrabold text-[#071632] shadow-sm sm:min-h-0 sm:flex-row sm:items-center sm:justify-start sm:gap-3 sm:p-4">
