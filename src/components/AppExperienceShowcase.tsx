...
const pillars = [
  ['Community', 'Posts, groups, pages, and local updates', 'fa-people-group'],
  ['Marketplace', 'Bazaar, housing, jobs, and services', 'fa-store'],
  ['Media', 'Hamro TV, videos, and reels', 'fa-play'],
  ['Discovery', 'Events, businesses, messages, and search', 'fa-location-dot'],
];
...
const panels = [
  ...
  },
  {
    ...
    alt: 'uNepal marketplace artwork',
...
export default function AppExperienceShowcase() {
  return (
    <section id="experience" className="relative isolate overflow-hidden bg-[#071632] py-14 text-white sm:py-20 lg:py-24">
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <FadeIn className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-base font-semibold leading-8 text-white/68 sm:text-lg">
            uNepal brings social, community, marketplace, business, events, and media into one app-style experience for Nepalese everywhere.
          </p>
        </FadeIn>

        <FadeIn delay={0.1} className="relative mx-auto mt-12 min-h-[560px] max-w-6xl sm:mt-16 lg:min-h-[650px]">
          ...
          {panels.map((panel) => (
            ...
            >
              <div className="relative aspect-[16/10] overflow-hidden rounded-[1.05rem]">
                <Image src={panel.src} alt={panel.alt} fill sizes="40vw" className="object-cover" />
              </div>
            </div>
          ))}
            ...
            <div className="relative overflow-hidden rounded-[1.55rem] bg-[#F8FAFC] p-4 text-[#071632] sm:p-5">
              ...
              <div className="relative rounded-[1.25rem] bg-white p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs font-bold text-[#65676B]">Connected tools for everyday community life</p>
                  </div>
                </div>
              </div>
              ...
              <div className="relative mt-4 grid grid-cols-2 gap-3">
                {pillars.map(([title, detail, icon], index) => (
                  <div
                    key={title}
                    className="min-h-[128px] rounded-[1.2rem] border border-[#E4E6EB] bg-white p-4 text-left shadow-sm"
                  >
                    <span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${index % 2 === 0 ? 'bg-[#FFF2F4] text-brand-primary' : 'bg-[#F2F5FA] text-brand-blue'}`}>
                      <i className={`fa-solid ${icon}`} />
                    </span>
                    <p className="mt-4 text-sm font-extrabold text-[#071632]">{title}</p>
                    <p className="mt-1 text-[11px] font-semibold leading-4 text-[#65676B]">{detail}</p>
                  </div>
                ))}
              </div>
            </div>
